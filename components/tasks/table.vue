        ...
        <div v-if="tasks">
            ...
            >
                ...
                <template v-slot:item.users="{ item }">
                  <div v-for="(user, index) in item.users" :key="index" class="smallRow">
                    {{ user.name }}
                  </div>
                </template>
                ...
                <template v-slot:item.files="{ item }">
                    ...
                    <v-icon v-if="item.result !== null && item.status_id === 3" color="#66bb6a" small>mdi
                        mdi-note-edit-outline
                    </v-icon>
                    <v-icon v-if="(item.results_count === 0 && item.result === null) && item.status_id === 3"
                            color="#ef5350" small>mdi mdi-help
                    </v-icon>
                ...
                <template v-slot:item.status="{ item }">
                    <v-chip v-if="item && !item.formatted_execution_date && !item.started_work_date"
                            ...
                            outlined
                    >
                        <v-icon dark left small>mdi mdi-invoice-text-plus</v-icon>
                        В ожидании...
                    </v-chip>
                    ...
                    >
                        <v-icon dark left small>mdi mdi-check-all</v-icon>
                        {{ item.status.title }}
                    </v-chip>
                    <v-chip
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Исполнитель', value: 'users', sortable: false},
        ...
        methods: {
            ...
            async getTasksList(filter_data, user_id, page) {
                if (this.currentUserId) {
                    await this.$store.dispatch('tasks/GET_TASKS_FROM_API', [filter_data, 0, page]);
                }
            },
            ...
            async submitFilter(data) {
                ...
                this.getTasksList(data, 0, this.currentPage)
            },
            async resetFilter() {
                this.getTasksList([], 1);
                this.$store.commit('tasks/STORE_CURRENT_PAGE', 1);
