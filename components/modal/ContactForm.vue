...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async removeContactLink(related_contact_id, contact_id) {
                this.removeLoader = true;
                const formData = new FormData();
                formData.append('contact_id', contact_id);
                formData.append('related_contact_id', related_contact_id);
                const response = await this.$axios.$post('/api/contacts-contact-relation/remove', formData);
                if (response) {
                    await this.$store.dispatch('contacts/GET_CONTACT_FROM_API', contact_id);
                    this.removeLoader = false;
                }
