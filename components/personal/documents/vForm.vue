    ...
    export default {
        ...
        computed: {
            ...mapState('documents', ['groups', 'task_types', 'form_loading', 'error', 'errors', 'success',
                'open_document_id', 'document', 'open_document_events',
            ]),
            ...
            creator() {
                if (this.document && this.document.formatted_created_at) {
                    return this.$auth.user.name + ", " + this.document.formatted_created_at
                } else {
        ...
        methods: {
            ...
            async close() {
                ...
                await this.$store.dispatch('documents/GET_DOCUMENTS_FROM_API', [[], this.currentUserId, 1]);
            },
            async save() {
                const formData = new FormData();
